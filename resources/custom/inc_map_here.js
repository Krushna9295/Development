...
var map_pin_image_hover = base_url+'themes/backend/images/select_bike_icon.png';
var stand_map_pin_image_hover = base_url+'themes/backend/images/select_bike_icon.png';
var map_pin_image = base_url+'themes/backend/imagesresources/images/select_icon_pin.png';
var map_102_pin_image = base_url+'themes/backend/images/amb-green_102.png';
var stand_map_pin_image = base_url+'resources/images/select_icon_pin.png';
var $MapPins = {
...
var $MapPins = {
    ...
    2:'http://localhost/tdd_mysql/resources/images/red_map_pin.png',
    1:'http://localhost/tdd_mysql/resources/images/red_map_pin.png',
    ...
    $("#inc_ext_map_address").autocomplete({
       ...
       // console.log(  ui.item );
         ...
         source: function (request, response) {
             ...
             $.ajax({
               // url: "https://places.ls.hereapi.com/places/v1/autosuggest?in=34.083813%2C74.809463%3Br%3D5000000&size=1000&tf=plain&addressFilter=stateCode%3DMH&X-Mobility-Mode=drive&Accept-Language=en&app_id=hSm0zJfek39BQxeXGRYZ&app_code=46aPkrbrHTb7tEdxSzLMiA&apiKey=yrjPrIYd0xU9KJpe1xlaR1_K1wFrwc9U-_-99n040JQ",
               url: "https://places.ls.hereapi.com/places/v1/discover/search?in=19.0760%2C72.8777%3Br%3D4717093&addressFilter=stateCode%3DMH&recd=false&size=100&tf=html&X-Mobility-Mode=drive&Accept-Language=en&app_id=hSm0zJfek39BQxeXGRYZ&app_code=46aPkrbrHTb7tEdxSzLMiA",
                 data: { q: request.term },
                 dataType: "json",
                 success: function ($data) {
...
function set_inc_add_details(place,map_tagId){
    ...
    // });
    var amb_type = "1";
    // var $inc_type = $('#purpose_call').val();
    var inc_type = "NON_MCI";
    // console.log(inc_type);
    // var $inc_type = "";
    var $dist_id = $("#incient_dist .mi_autocomplete").val();
     
    get_inc_ambu(inc_type,amb_type,p_lat,p_lng);
    
    // $('#get_ambu_details').attr('href',base_url+'calls/get_inc_ambu?showprocess=yes&inc_type='+$inc_type+'&amb_tp='+$amb_type+'&lat='+p_lat+'&lng='+p_lng);
   ...
   // init_auto_address('INCIDENT_MAP');
    ...
    //console.log(place);
        ...
        loc_street = place.address.street;
        


    data_qr = "dt_auto_addr=inc_auto_addr&dt_rel=incient&dt_state=" + dt_state + "&dt_dist=" + dt_dist + "&dt_thl=" + dt_thl + "&dt_city=" + dt_city + "&dt_area=" + dt_area + "&dt_pin=" + dt_pin + "&loc_state=" + loc_state + "&loc_dist=" + loc_dist + "&loc_city=" + loc_city + "&loc_area=" + loc_area + "&loc_thl=" + loc_thl + "&loc_pin=" + loc_pin + "&dt_lat=" + p_lat + "&dt_log=" + p_lng;

    $('#inc_pincode').val(loc_pin);
    $('#lane_street').val(loc_street);
    $('#inc_area').val(loc_area);
    let _url = base_url+`/calls/manage_addr`;
    $.ajax({
        type: 'GET',
        url: _url,
        data: {loc_state: loc_state,dist:loc_dist,thl:loc_thl,city:loc_city},
        dataType: 'json',
        success: function (data) {
            console.log(data); 
            if(data){
                $("#state").val(data.state[0].st_code).prop('selected', true);
                if(data.dist != ""){
                    $("#district").val(data.dist[0].dst_code).prop('selected', true);
                }
                if(data.tahshil != ""){
                    $("#tahshil").val(data.tahshil[0].thl_code).prop('selected', true);
                }
                if(data.city != ""){
                    $("#city").val(data.city[0].cty_id).prop('selected', true);
                }
            }   
        }
    });
...
//        }
    ...
    $('#container').on('click', '#inc_recomended_ambu .amb_type_checkbox' , function () {
        ...
        if ($("input[name='addtess_type']:checked").val() == 'google_addres') {
            ...
            $('#get_ambu_details').attr('href',base_url+'calls/get_inc_ambu?inc_type='+$inc_type+'&amb_tp='+$amb_type+'&lat='+p_lat+'&lng='+p_lng+'&district_id='+$dist_id);
            $("#get_ambu_details").click();
            }
        }else{
            $('#get_ambu_details').attr('href',base_url+'calls/get_inc_ambu?inc_type='+$inc_type+'&amb_tp='+$amb_type+'&lat='+p_lat+'&lng='+p_lng+'&district_id='+$dist_id);
    ...
    $('#container').on('click', '#inc_recomended_ambu .amb_status_checkbox' , function () {
        ...
        $('#get_ambu_details').attr('href',base_url+'calls/get_inc_ambu?inc_type='+$inc_type+'&amb_tp='+$amb_type+'&lat='+p_lat+'&lng='+p_lng+'&inc_status='+$status+'&district_id='+$dist_id);
        $("#get_ambu_details").click();
    });
...
function get_inc_ambu(inc_type,amb_type,p_lat,p_lng){
    // alert(inc_type)
    $.ajax({
        type: 'GET',
        url: base_url+`/calls/get_inc_ambu`,
        data: {inc_type: inc_type,amb_type:amb_type,p_lat:p_lat,p_lng:p_lng},
        dataType: 'json',
        success: function (new_var) {
            console.log(new_var);
            var raw;
            var info;
            $.each( new_var, function( key, value ) {
                // console.log(new_var[key].base_loc[0]);
                var ambId =  'Search_Amb_'+new_var[key].amb_id;
                var data_amb_id = new_var[key].amb_id;
                raw += "<tr class='searched_ambu_item' id = '"+ambId+"' data-amb_id='"+data_amb_id+"' data-rto-no='"+new_var[key].amb_rto_register_no+"' data-lat='"+new_var[key].amb_lat+"' data-lng='"+new_var[key].amb_log+"' data-title='"+new_var[key].hp_name+"' data-amb_type='"+new_var[key].amb_type+"' data-amb_status='"+new_var[key].amb_status+"' data-amb_geofence='"+new_var[key].geo_fence+"'>"+
                    "<td>"+ new_var[key].hp_name +"</td>"+
                    "<td>"+ new_var[key].amb_rto_register_no +"</td>"+
                    "<td>" + new_var[key].road_distance + "</td>"+
                    "<td>" + new_var[key].duration + "</td>"+
                    "<td>" + new_var[key].amb_default_mobile + "<a href='#' class='pt-2' style='margin-left:3px;'><i class='fas fa-mobile-alt '></i></a></td>"+
                    "<td>"+ new_var[key].amb_pilot_mobile +"<a href='#' class='pt-2' style='margin-left:3px;'><i class='fas fa-mobile-alt '></i></a></td>"+
                    "<td>"+ new_var[key].ambs_name+"</td>"+
                    "<td>"+ new_var[key].ambt_name+"</td>"+
                    "<td><input type='checkbox' class='amb_check_box' id='check"+new_var[key].amb_rto_register_no+"' name='amb_reg_no[]' data_lat=" + new_var[key].amb_lat+" data_lng=" + new_var[key].amb_log +" value="+new_var[key].amb_rto_register_no+" > Assign</td>"+
                    "<td style='display: none;'><div class='ambu_pin_info'><strong>"+ new_var[key].amb_rto_register_no +"</strong><br><strong>Distance: </strong>"+ new_var[key].road_distance +"<br><strong>Duration: </strong>"+ new_var[key].duration +"<br><strong>RTO No: </strong>"+ new_var[key].amb_rto_register_no +"<br><strong>Mobile No: </strong>"+ new_var[key].amb_default_mobile +"</div></td>"+
                    "</tr>";
                if(typeof google != 'undefined'){
                    plot_amb_on_map(new_var[key].amb_lat, new_var[key].amb_log, info, map_mark_pin);
                    update_ambulance_inc_map();
                }
            });
            // $('.ambu_pin_info').append(info);
            $('#inc_ambu_list').append(raw);
            update_ambulance_inc_map();
        }
    });
}